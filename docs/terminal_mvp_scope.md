...
output. Script tags are specific for Latin, Greek, Cyrillic, Hebrew, Arabic,
...
emoji/common runs. The shaping adapter also tries configured fallback font faces
in the same order as `textrender` when the primary face shapes a run with
`.notdef` glyphs.
